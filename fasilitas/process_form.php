...
$query1 = "SELECT * FROM t_data_pinjam p join t_fasilitas m on p.p_barang = m.t_id_fasilitas where (p_m_pinjam BETWEEN '" . $p_m_from . "' AND  '" . $p_m_to . "') AND p_verif = '1' AND  p_golongan = 'fasil' ORDER by p_counter ASC";
        $result1 = $dbh->query($query1)->fetchAll();
    $query2 = "select m.m_nama_barang from t_data_pinjam  p join t_fasilitas m on p.p_barang = m.t_id_fasilitas";
...
<img src="img/kop_laporan_fasilitas.png" width="100%">
    <thead>
        <tr>
            <th>No.</th>
