...
//           $result1 = $dbh->query($query1)->fetch();  where t_nama_mobil =''
           ...
           $query2= "SELECT * FROM t_data_pinjam";
           $result2 = $dbh->query($query2)->fetch();
//           $query_mobil = "SELECT m_id_mobil FROM t_mobil";
//$result_mobil = $dbh->query($query_mobil)->fetch(); 
           
          
//           $query_pinjam = "SELECT p.p_m_pinjam FROM t_data_pinjam p where p_golongan = 'trans' ";
//           $result_pinjam = $dbh->query($query_pinjam)->fetchAll();
         
//         for($i = 1; $i <= $result6['ruang']; $i++ ){
//             $result_pinjam = array();
//             $query_pinjam = "SELECT p.p_m_pinjam FROM t_data_pinjam p where p_golongan = 'trans' AND p_barang = '$i'";
//           $result_pinjam[] = $dbh->query($query_pinjam)->fetchAll();
//         }  
           ...
           if($jut == 2){
               ?>
                            <table id="example1" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <center>
                                    <div class="panel-heading">Table Informasi Transportasi</div>
                                </center>
                                <thead>
                                    <tr>
                                        <?php foreach ( $result1 as $r1 ) { 
                            $p_barang = $r1['m_id_mobil'];
                       $query_pinjam = "SELECT * FROM t_data_pinjam p where p_golongan = 'trans' AND p_barang= '$p_barang' AND p_m_kembali >= '$p_timest' ";
                       $result_pinjam = $dbh->query($query_pinjam)->fetchAll();
                            ?>
                                            <th>
