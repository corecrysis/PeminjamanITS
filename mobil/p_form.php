...
$query = "SELECT * FROM t_user where m_id_user='$jut'";
     ...
     $hasil_jx = $result_jx['counter_trans'];
               ...
               if($result['m_id_user']==3){
if ($uploadOk == 1){
            if (move_uploaded_file($tmp_gambar, $new_location1)){
               $p_harga     = $_POST['p_harga'];
                   $table = 't_data_pinjam'; // ubah ke nama tabel
        $field = '`p_unit`,`p_m_pinjam`,`p_m_kembali`,`p_tujuan`,`p_cp`,`p_harga`,`p_barang`,`p_golongan`, `p_timest`, `p_tmp_pict_1`, `p_verif`, `p_counter`'; // kolomnya, kalo > 1 pisahkan pakai koma
        $val = '?,?,?,?,?,?,?,?,?,?,?,?'; // ini sesuai jumlah kolomnya, pakai koma
        $array = array( $p_unit, $p_m_pinjam, $p_m_kembali, $p_tujuan, $p_cp, $p_harga,$p_barang,$p_golongan, $p_timest, $new_location1 , $status, $hasil_jx); // sesuai jumlah kolom juga
 
        $sth = $dbh->prepare( "INSERT INTO $table ($field) VALUES ($val)" );
        $input = $sth->execute( $array );
$query10 = "UPDATE t_counter SET  counter_trans = '$tambah_jx'";
